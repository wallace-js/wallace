...
### Detachers & Offset Trackers

A detacher attaches or removes conditional elements (those that use the `if` directive) from their parent. They use an offset tracker linked to the parent, which is also used by any repeaters:
...
- conditional children (the `if` directive)
- A repeater and siblings (as opposed to a repeater on its own)

Then an offset tracker (a plain Map object) is created for that node and stored in the component's stash. This object is passed into repeater constructors along with their initial index (the position where the repeat was declared). For conditional elements, the watch's shield gets a detacher object with its initial index, parent element key, and the stash index of the offset tracker:

```js
d: new Detacher(1, 2, 3);
```

The offset tracker stores a 0 or a -1 against each conditional element, and the number of children added for each tracker, which allows the detacher and the repeaters to calculate the correct index at which to insert items.
