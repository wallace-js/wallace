...
if (wallaceConfig.flags.allowRepeaterSiblings) {
  describe.each(permutations)("Normal element as sibling (%s)", (_, keyed) => {
    ...
    const Container = keyed
      ? () => (
          <div>
            <div>hello</div>
            <Child.repeat model={items} key="i" />
          </div>
        )
      : () => (
          <div>
            <div>hello</div>
            <Child.repeat model={items} />
          </div>
        );
  ...
  describe.each(permutations)("Repeater as sibling (%s)", (_, keyed) => {
    ...
    const Container = keyed
      ? () => (
          <div>
            <Child.repeat model={items1} />
            <Child.repeat model={items2} key="i" />
          </div>
        )
      : () => (
          <div>
            <Child.repeat model={items1} />
            <Child.repeat model={items2} />
          </div>
        );
    ...
    const Container = keyed
      ? () => (
          <div>
            ...
            <Child.repeat model={items1} key="i" />
